...
# ---- the anchor, replaced 17 September 2026 --------------------------------
# It was anchorAll: boxes (SOME, ALL), prompt "...has all of the objects",
# the ALL box correct. That forced the box-internal domain reading, because the
# open-box answer is available only under it. It also made the participant
# discriminate a SOME display from an ALL display and attach the label "all" to
# the fuller one, two screens before being asked about "some" -- which trains
# the stronger alternative rather than merely mentioning it. On the intro-class
# run of 2-3 Sept 2026 the scalar critical rate came out at 81% against Huang
# et al.'s 13%, and criticalOneSet, where the domain loophole is closed by
# construction, came out at 83%, so the loophole was not doing any work while
# the alternative plainly was. See archive/2026-09-intro-run/.
#
# anchorHalf shows 3-1 and 1-3 and asks for HALF, so neither open box matches
# and the covered box is correct. It rewards none of NONE, SOME or ALL, which
# are the configurations the critical trials are built from, and it never
# utters "all". Verifying it requires counting a whole box.
#
# What it gives up is the domain diagnostic, and that is structural rather than
# a bad choice of numbers: forcing the box-internal reading requires an
# open-box answer, and an open-box answer trains a word-to-picture mapping.
# The domain control therefore moves into the item design. criticalOneSet puts
# every object in one box, so the two readings coincide there and it is the one
# critical trial where a lower-bounded answer is unambiguously lower-bounded.
# It is the trial of record; a gap between it and the other two criticals is
# now the signature of whole-display quantification.
SCALAR = {"anchor":"anchorHalf", "probeEarly":"probeEarly", "critical":"critical",
...
SCALAR_BOXES = {"critical": ("NONE","ALL"),   # no subset match -> covered box
                ...
                "anchorHalf":("THREEONE","ONETHREE")}  # neither is half -> covered
